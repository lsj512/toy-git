/*
 * @author: LUOCAN
 * @create: 2021-05-06 14:38 PM
 * @license: MIT
 * @lastAuthor: LUOCAN
 * @lastEditTime: 2021-05-14 14:29 PM
 * @desc: 审核失败
 */
...
function Alldata(data) {
  ...
  userdata = data[0];
  // console.log(busdata);
  // 遍历出数据 找到status为3的数据为审核失败的
  busdata.forEach((ele) => {
    if (ele.status == 3) {
      ele.checked = false;
      faildata.push(ele); // 存放到新的数组中进行遍历渲染
    }
  });
  rander();
  breakPage();
 ...
 */
function breakPage() {
  allPage = Math.ceil(faildata.length / step); //向下取整数
...
function rander() {
  ...
  if (faildata.length == 0) {
    ...
    // 写入当前成功的条数
    fal.innerHTML = faildata.length;
  }
  // 一页的数据
  const ranerdata = faildata.slice(pageIndex * step, (pageIndex + 1) * step);
  ...
  ranerdata.forEach((ele) => {
    ...
    <div class="status">
      <P class="success">审核失败</P>
    </div>
    <div class="operate">
      <P><a href="javascript:;" class="cause">失败原因</a></P>
      <P><a href="../html/audit_fail_upd.html" class="order" id = "${
        ele.order_id
      }">修改</a></P>
    </div>`;
  ...
  // 获取失败原因按钮的元素
  let faill = getall(".cause");
  // console.log(faill);
  // 失败原因的点击事件
  faill.forEach((ele) => {
    ele.onclick = function () {
      get(".yy-fail").style.cssText = "display:block";
    };
  });
  failupa(ranerdata);
}
...
// TODO 预约失败确认事件
let enter = get(".enter"); //获取预约失败的确认按钮元素
enter.onclick = function () {
  get(".yy-fail").style.cssText = "display:none";
};

// TODO 修改按钮
let str = []; // 存放缓存的数组
/**
 * @param {*} failupa 修改按钮事件
 * @param {*} fd 渲染失败的数据
 */
function failupa(ran) {
  ran.forEach((ele) => {
    let i = ele.order_id; //中间变量
    let id = document.getElementById(i); // 获取修改元素
    id.onclick = () => {
      console.log(111);
      if (ele.order_id == id.id) {
        // ! 缓存的存储
        str = JSON.stringify(ele); // 转换为字符串
        localStorage.setItem("item", str); //缓存数据
      }
    };
  });
}
...
//TODO 点击数据列表复选框
function ch(id) {
  let newdata = faildata.slice(pageIndex * step, (pageIndex + 1) * step);
...
get("#batchon").onclick = function () {
  ...
  deldata.forEach((ele) => {
    if (ele.checked == true) {
      let index = faildata.indexOf(ele);
